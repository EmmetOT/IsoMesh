        ...
        public override void OnInspectorGUI()
        {
            ...
            if (m_isVoxelSettingsOpen = EditorGUILayout.Foldout(m_isVoxelSettingsOpen, Labels.VoxelSettings, true))
            {
                using (EditorGUILayout.VerticalScope box = new EditorGUILayout.VerticalScope(EditorStyles.helpBox))
                {
                    ...
                    {
                        m_setter.DrawFloatSetting(Labels.OcttreeRootWidth, m_serializedProperties.OcttreeRootWidth, min: 0.005f, max: 100000f/*, onValueChangedCallback: m_sdfGroupMeshGen.OnOcttreeRootWidthChanged*/);
                        m_setter.DrawIntSetting(Labels.OcttreeMaxNodeDepth, m_serializedProperties.OcttreeMaxNodeDepth, min: 2, max: 10, onValueChangedCallback: m_sdfGroupMeshGen.OnOcttreeMaxNodeDepthChanged);
                        m_setter.DrawFloatSetting(Labels.OcttreeNodePadding, m_serializedProperties.OcttreeNodePadding, min: 0f, max: 10f, onValueChangedCallback: m_sdfGroupMeshGen.OnOctreeNodePaddingChanged);
